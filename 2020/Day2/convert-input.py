...
lineFmt = re.compile( "(\d+)-(\d+) (.): (.+)" )

def charToNat( c ):
    # a = 0 based numbering
    return "{}".format( ord( c ) - ord( 'a' ) )
...
def stringToVec( s ):
    ...
    for c in s:
        out.append( " (vec:: " )
        out.append( charToNat( c ) )
        closeParens += 1
    out.append( " vecnil")
    out.append( ")" * closeParens )
    return "".join( out )
        
print( "(claim day2 (List Input))" )
print( "(define day2" )
closeParens = 1

for line in input.split( "\n" ):
    m = lineFmt.match( line )
    if m is None:
        print( "Error on '{}'".format( line ) )
        break
    
    print( "  (:: (make-input {} {} {} {})".format(
        m.group( 1 ),
        m.group( 2 ),
        charToNat( m.group( 3 ) ),
        stringToVec( m.group ( 4 ) )))
    closeParens += 1

print( "   nil" + ")" * closeParens )
